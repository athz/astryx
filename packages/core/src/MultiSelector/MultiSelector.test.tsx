...
describe('MultiSelector dropdown row theme targets', () => {
  ...
  it('renders astryx-multi-selector-option-label on the fallback label span', async () => {
    const user = userEvent.setup();
    render(
      <MultiSelector
        label="Fruit"
    ...
    const options = screen.getAllByRole('option', h);
    for (const option of options) {
      const label = option.querySelector('.astryx-multi-selector-option-label');
      expect(label).not.toBeNull();
    }
    // The Apple option's label span carries the target and its text.
    const appleLabel = screen
      .getAllByRole('option', h)[0]
      .querySelector('.astryx-multi-selector-option-label');
    expect(appleLabel).toHaveTextContent('Apple');
  });
  ...
  it('does not render the fallback label when renderOption is used', async () => {
    ...
    expect(option).toHaveClass('astryx-multi-selector-option');
    expect(option.querySelector('.astryx-multi-selector-option-label')).toBeNull();
  ...
  it('exposes the row, checkbox, and select-all divider targets to defineTheme', () => {
    ...
    expect(css).toContain('.astryx-multi-selector-option-label {');
